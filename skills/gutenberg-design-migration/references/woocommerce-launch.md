...
## 7. What WordPress leaves open to the public
...
## 8. Launch checklist beyond the theme
...
- [ ] Author slug closed on all three routes: `/wp/v2/users`, `?author=1` and
      the SEO plugin's author sitemap (section 7: closing the REST endpoint
      alone clears the scan and leaves the leak)
- [ ] `/xmlrpc.php` denied before PHP runs, in `.htaccess` or the WAF
      (section 7: `xmlrpc_enabled` leaves `pingback.ping`, and a mu-plugin has
      booted WordPress before it can refuse)
